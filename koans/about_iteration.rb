...
class AboutIteration < EdgeCase::Koan
  ...
  def test_extra_credit
    result = [2, 3, 4].my_inject(0) { |sum, item| sum + item }
    assert_equal 0+2+3+4, result

    result2 = [2, 3, 4].my_inject(1) { |sum, item| sum * item }
    assert_equal 1*2*3*4, result2
  end
  ...
  def test_all_iteration_methods_work_on_any_collection_not_just_arrays
    ...
    assert_equal [11, 12, 13], result # NOTE: not (11..13)
  ...
  #
  # The first one doesn't need to call File#close explicitly.
